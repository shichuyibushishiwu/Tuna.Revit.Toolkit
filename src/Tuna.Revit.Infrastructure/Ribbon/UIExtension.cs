...
using Tuna.Revit.Infrastructure.Ribbon.Proxy;


namespace Tuna.Revit.Infrastructure.Ribbon;
...
public static class UIExtension
{
    ...
    /// </summary>
    /// <typeparam name="T">外部命令，必须是一个自定义类型，且继承于<see cref="IExternalCommand"/>,且必须存在一个无参的构造函数</typeparam>
    ...
    {
        RevitApplicationContext.Instance.Assembly = Assembly.GetCallingAssembly();
        return panel.InternalCreateComboBox(name, handle);
    }
    ...
    public static PushButton CreatePushButton<T>(this PulldownButton pulldownButton, Action<PushButtonData>? handle = null) where T : class, IExternalCommand, new()
    {
        ...
        return pulldownButton.CreatePushButton(typeof(T), handle);
    }
